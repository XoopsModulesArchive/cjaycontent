...
switch ($op) {
        ...
        case 'add':
                ...
                $form_comment = $myts->addSlashes($form_comment);
                $form_address = $myts->addSlashes($form_address);
                $cont = $myts->addSlashes($cont[1]);
                $content = addslashes((string)$cont);
                $date = time();
                if ($form_hide) {
                    $form_hide = 1;
                } else {
        ...
        case 'mainmenu':
                       if ('yes' == $mainm) {
                           ...
                           $result = $xoopsDB->query('SELECT id,title,weight FROM ' . $xoopsDB->prefix() . '_cjaycontent WHERE NOT (weight=0) ORDER BY weight');

                           $oldfile = './../xoops_version.php';

                           $old = fopen($oldfile, 'rb');

                           $new = fopen($oldfile . '.new', 'wb');
                           ...
                           while ($line = fgets($old, 4096)) {
                               ...
                               $fp = fopen('./../xoops_version.php', 'a+b');
                               ...
                               fwrite($fp, "\$modversion['templates'][1]['file'] = 'cc_index.html';\n");

                               fwrite($fp, "\$modversion['templates'][1]['description'] = 'cc main template file';\n");

                               fwrite($fp, "\$modversion['hasComments'] = 1;\n");

                               fwrite($fp, "\$modversion['comments']['pageName'] = 'index.php';\n");

                               fwrite($fp, "\$modversion['comments']['itemName'] = 'id';\n");

                               while ($mm_item = $xoopsDB->fetchArray($result)) {
                                   fwrite($fp, "\$modversion['sub'][$y]['name']=\"" . htmlspecialchars($mm_item['title'], ENT_QUOTES | ENT_HTML5) . "\";\n");

                                   fwrite($fp, "\$modversion['sub'][$y]['url']=\"index.php?id=" . htmlspecialchars($mm_item['id'], ENT_QUOTES | ENT_HTML5) . "\";\n");
...
switch ($op1) {
        case 'delconfirm':
                ...
                echo '<center><h4>' . _CC_DEL_REALLY . '</h4><br>' . $fc_item['id'] . ' <b>|</b> ' . htmlspecialchars($fc_item['title'], ENT_QUOTES | ENT_HTML5)
                     . ' <b>|</b> ' . htmlspecialchars($fc_item['comment'], ENT_QUOTES | ENT_HTML5)
                     . "<br><form action='index.php' method='post'><input type='hidden' name='id' value='" . $iddel . "'><input type='hidden' name='op' value='del'><input type='submit' value='" . _CC_DELETE . "'>&nbsp;<input type='button' value='" . _CANCEL . "' onclick='javascript:history.go(-1);'></form></center>";
                CloseTable();
                fc_footer();
                break;
        case 'edconfirm':
            ...
            echo '<form name="Edit Content" action="./index.php" method="post"><div align="center"><h4>' . _CC_EDIT_HEADER . '</h4></div><table border="0" cellpadding="2" cellspacing="2" width="95%">
                ...
                <tr>
                                                ...
                                                <td><input type="text" name="form_adress" size="20" maxlength="40"  value="' . htmlspecialchars($fc_item['adress'], ENT_QUOTES | ENT_HTML5) . '" tabindex="2"></td>
                </tr>
                <tr>
...
function fc_admin_list()
{
    ...
    while ($fc_item = $xoopsDB->fetchArray($result)) {
		...
		<td>' . htmlspecialchars($fc_item['comment'], ENT_QUOTES | ENT_HTML5) . '</td>
		<td>' . $fc_item['hide'] . '</td>
		<td>' . $fc_item['hits'] . '</td>
...
function fc_admin_add()
{
    ...
    if ($dir = opendir('./../content/')) {
        while (false !== ($file = readdir($dir))) {
            if (is_file('./../content/' . $file) and ('DO_NOT_DELETE.php' != $file)) {
...
/*function fc_admin_edit($idedit){
        ...
        echo "<form name=\"Edit Content\" action=\"./index.php\" method=\"post\"><div align=\"center\"><h4>"._CC_EDIT_HEADER."</h4></div><table border=\"0\" cellpadding=\"2\" cellspacing=\"2\" width=\"95%\">
                ...
                <tr>
                        <td align=\"right\">"._CC_TITLE.":</td>
                                                <td><input type=\"text\" value=\"".htmlspecialchars($fc_item['title'])."\" name=\"form_title\" size=\"50\" tabindex=\"1\"> </td>
                </tr>
                <tr>
                                                <td align=\"right\">"._CC_ADRESS.":</td>
                                                <td><input type=\"text\" name=\"form_adress\" size=\"20\" maxlength=\"40\"  value=\"".htmlspecialchars($fc_item['adress'])."\" tabindex=\"2\"></td>
                </tr>
                <tr>
                                                <td align=\"right\">"._CC_COMMENT.":</td>
                                                <td><input type=\"text\" value=\"".htmlspecialchars($fc_item['comment'])."\" name=\"form_comment\" size=\"100\" tabindex=\"3\"></td>
                </tr>
                <tr>
                                                <td align=\"right\">"._CC_HIDE.":</td>
                                                <td><input type=\"checkbox\" value=\"1\" name=\"form_hide\" tabindex=\"4\" ".$hide_checked."> "._CC_ADD_HIDELONG."</td>
...
function mainmenu_admin()
{
    ...
    while ($cc_item = $xoopsDB->fetchArray($result)) {
        echo '<tr>
		<td>' . htmlspecialchars($cc_item['title'], ENT_QUOTES | ENT_HTML5) . '</td>
		<td>' . htmlspecialchars($cc_item['comment'], ENT_QUOTES | ENT_HTML5) . '</td>
		<td>' . htmlspecialchars($cc_item['adress'], ENT_QUOTES | ENT_HTML5) . '</td>
