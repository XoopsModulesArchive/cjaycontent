...
class Web
{
    ...
    public $Apache_link = "<A HREF='http://www.apache.org'>Apache</A>";

    public $Xitami_link = "<A HREF='http://www.imatix.com'>Xitami</A>";

    public $VMware_link = "<A HREF='http://www.vmware.com'>VMware</A>";

    public $Homepage_url = 'http://www.ekenberg.se/php/ide/';

    public $Screenshot_main_url = 'http://www.ekenberg.se/php/ide/images/ide.php-main_window.gif';

    public $Screenshot_options_url = 'http://www.ekenberg.se/php/ide/images/ide.php-options.gif';

    public $tar_gz_filename = 'ide.php-1.5.tar.gz';

    public $zip_filename = 'ide.php-1.5.zip';
    ...
    public function __construct()
    {
        ...
        while (list($label, $header) = each($this->Text->Info_sections)) {
            ...
            if ($label == $_GET['label']) {
                print $this->Out->begin_invisible_table('85%', ["CELLPADDING='1'", "CELLSPACING='0'", "ALIGN='center'", "BGCOLOR='#000000'"]);

                print "<TR><TD>\n";

                print $this->Out->begin_invisible_table('100%', ["CELLPADDING='20'", "CELLSPACING='0'", "ALIGN='center'", "BGCOLOR='{$this->Out->Box_bgcolor}'"]);
...
'contact' => 'Contact',
    ...
    public function news()
    {
        if (file_exists('./Changes.txt')) {
            ...
            for ($i = 0, $iMax = count($changes_array); $i < $iMax; $i++) {
                $changes_array[$i] = preg_replace('^([-0-9]+)[[:space:]]+(.+)', '<small>\\1</small> \\2', htmlentities($changes_array[$i], ENT_QUOTES | ENT_HTML5));
            }
        } else {
    ...
    public function download()
    {
        ...
        $ret .= "<P>Ide.php is distributed as a compressed archive, choose the kind you prefer:</P>
            <UL>
    ...
    public function num_downloads($filetype)
    {
...
class Page
{
    public $Bgcolor = '#FFE56A';

    public $Box_bgcolor = '#FFFFDD';

    public $Link_color = '#0A0AA0';

    public $Alink_color = '#0000CC';

    public $Vlink_color = '#464686';

    public function start_box_table($width)
    {
        $ret .= $this->begin_invisible_table($width, ["CELLPADDING='1'", "CELLSPACING='0'", "ALIGN='center'", "BGCOLOR='#000000'"]);

        $ret .= "<TR><TD>\n";

        $ret .= $this->begin_invisible_table($width, ["CELLPADDING='12'", "CELLSPACING='0'", "WIDTH='100%'", "BGCOLOR='{$this->Bgcolor}'"]);

        return ($ret);
    }

    public function end_box_table()
    {
    ...
    public function begin_invisible_table($width, $attr = '')
    {
        $ret = "<TABLE WIDTH='$width' BORDER='0' ";

        $ret .= (is_array($attr) ? implode(' ', $attr) : null) . ">\n";

        return ($ret);
    }

    public function end_invisible_table()
    {
        $ret = "</TABLE>\n";
    ...
    public function __construct()
    {
        $this->CSS_code = "<STYLE TYPE='text/css'>
   <!--
   A {
